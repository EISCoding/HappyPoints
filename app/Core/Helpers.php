...
function appConfig(): array { static $c=null; if($c===null){$c=require BASE_PATH.'/config/app.php';} return $c; }
function appName(): string { $c = appConfig(); return (string)($c['name'] ?? 'App'); }
function appUrl(string $path = ''): string { $base = rtrim((string)(appConfig()['base_url'] ?? ''), '/'); $path = '/' . ltrim($path, '/'); return $base . ($path === '/' ? '' : $path); }
